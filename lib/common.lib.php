...
{
    global $g5, $DM, $qstr, $board;

    $file['count'] = 0;
    $sql = " select * from {$DM['BF_TABLE']} where bd_id = '$bo_table' and uid = '$wr_id' order by bf_no ";
...
// 관리자인가?
function is_route($mb_id)
{
    global $config, $group, $board, $route;

    if (!$mb_id) return '';

    $is_authority = '';

    if ($route['sc_admin'] == $mb_id){
        $is_authority = 'super';
    } 

    return run_replace('is_route', $is_authority, $mb_id);
}

// 지점관리자인가?
function is_branch($mb_id)
{
    global $config, $group, $board, $branch;
...
function view_file_link2($file, $width, $height, $content='')
{
	...
	*/
    ...
    if (preg_match("/\.({$config['cf_image_extension']})$/i", $file) && isset($board['bo_table'])) {
        ...
        $img .= '<img src="'.G5_DATA_URL.'/file/qna/'.urlencode($file).'" alt="'.$content.'" '.$attr.'>';
        $img .= '</a>';
...
function get_manager($manager_id, $fields='*', $is_cache=false){

	global $DM;
    
    $sql = " select $fields from DM_T_MANAGER where manager_id = TRIM('$manager_id') "; 

	return sql_fetch($sql);

}

function get_branch_id($manager_id, $fields='*', $is_cache=false){

	global $DM;
    
    $sql = " select $fields from DM_T_BRANCH where branch_manager_id = TRIM('$manager_id') "; 
