...
function icon($act, $link='', $target='_parent')
{
    ...
    $icon = '<img src="'.G5_BRANCH_PATH.'/img/icon_'.$img[$act].'.gif" title="'.$act.'">';
    if ($link)
...
function admin_referer_check($return=false)
{
    ...
    if( $p['path'] && ! preg_match( '/\/'.preg_quote(G5_BRANCH_DIR).'\//i', $p['path'] ) ){
        $msg = '올바른 방법으로 이용해 주십시오';
    }
...
if (!$branch['branch_manager_id'])
{
    if($is_route=='super'){
		session_destroy();
		alert('최고 관리자 아이디로 지점 관리자는 접근 할 수 없습니다','/_route');
	}else{
		alert('로그인 하십시오.', G5_BBS_URL.'/branch.php?url=' . urlencode(correct_goto_url(G5_BRANCH_URL)));	
	}
	
	
}
else if ($is_branch != 'super')
{
    $auth = array();
    $sql = " select au_menu, au_auth from {$g5['auth_table']} where mb_id = '{$branch['branch_manager_id']}' ";
    $result = sql_query($sql);
    for($i=0; $row=sql_fetch_array($result); $i++)
    {
...
$admin_key = md5($branch['branch_reg_dt'] . get_real_client_ip() . $_SERVER['HTTP_USER_AGENT']);
if (get_session('ss_branch_key') !== $admin_key) {
...
while ($entry = $tmp->read()) {
    ...
    $amenu[$m[1]] = $entry;
    $menu_files[] = G5_BRANCH_PATH.'/'.$entry;
}
@asort($menu_files);
