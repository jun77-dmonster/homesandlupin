...
$files = glob(G5_BRANCH_PATH.'/css/admin_extend_*');
if (is_array($files)) {
    foreach ((array) $files as $k=>$css_file) {
        ...
        $css_file = str_replace(G5_BRANCH_PATH, G5_BRANCH_URL, $css_file);
...
{
    global $menu, $auth_menu, $is_branch, $auth, $g5, $sub_menu;
    ...
    for($i=1; $i<count($menu[$key]); $i++)
    {
        ...
        if ($is_branch != 'super' && (!array_key_exists($menu[$key][$i][0],$auth) || !strstr($auth[$menu[$key][$i][0]], 'r')))
            continue;
...
<header id="hd">
    ...
    <div id="hd_top">
       ...
       <div id="logo"><a href="<?php echo correct_goto_url(G5_BRANCH_URL); ?>"><img src="<?php echo G5_BRANCH_URL ?>/img/logo.png" alt="<?php echo get_text($config['cf_title']); ?> 관리자"></a></div>

        <div id="tnb">
            <ul>
                <li class="tnb_li"><a href="<?php echo G5_BRANCH_URL ?>/room_service.php" class="tnb_service">ROOM 호출</a></li>
                <li class="tnb_li"><button type="button" class="tnb_mb_btn">관리자<span class="./img/btn_gnb.png">메뉴열기</span></button>
                    <ul class="tnb_mb_area">
                        <li><a href="<?php echo G5_BRANCH_URL ?>/member_form.php?w=u&amp;mb_id=<?php echo $member['mb_id'] ?>">관리자정보</a></li>
                        <li id="tnb_logout"><a href="<?php echo G5_BBS_URL ?>/branch_logout.php">로그아웃</a></li>
                    </ul>
                </li>
            </ul>
        </div>
