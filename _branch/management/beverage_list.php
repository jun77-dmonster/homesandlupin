...
$sql_search = " where t1.branch_cd='{$branch['branch_cd']}' and t1.delete_fl='F' and t2.beverage_delete_fl='F' ";
		...
		<tr>
			...
			<td>
				<select class="small" name="sfl" style='width:200px;'>
					<option value="beverage_kor_nm" <?php echo ($sfl=="beverage_kor_nm")?"selected":""?>>상품이름(한글)</option>
					<option value="beverage_eng_nm" <?php echo ($sfl=="beverage_eng_nm")?"selected":""?>>상품이름(영문)</option>
				</select>
	...
	<div class="mall-products-view">
		...
		<div class="mall-orders-view">
			...
			<div class="table-tit-area">
				...
				<span class="table-tit">등록된 음료 <span class="num"><?php echo $total_count?></span> 건</span>
				...
				<div class="btn-wrap-left">
					...
					<button type="submit" name="act_button" value="선택품절" onclick="document.pressed=this.value"  class="crmBtn type-white">선택품절</button>
					<button type="submit" name="act_button" value="선택판매" onclick="document.pressed=this.value"  class="crmBtn type-white">선택판매</button>
			...
			<div class="content_item_bx">
				...
				<div class="tbl_head01 tbl_wrap">
					...
					for ($i=0; $row=sql_fetch_array($result); $i++) {
						//$s_mod = "<a href='./beverage_register.php?w=u&amp;beverage_cd={$row['beverage_cd']}' class='board_copy btn btn_02'>지점문의</a>";
					...
					<tr>
						...
						<td class="td_cnt"><?php echo ($row['sold_out_fl']=="F")?"판매중":"품절"?></td>
						<td class="td_date"><?php echo substr($row['branch_beverage_reg_dt'],0,10)?></td>
						<td class="td_mng">
							<button type="button" onclick="popQna2('<?php echo $DM['QNA_CODE6']?>','<?php echo $row['beverage_cd']?>')" class='board_copy btn btn_02'>지점문의</button>
						</td>
					</tr>
					<?php
					}
...
<div class="btn_fixed_top">
    <a href="./beverage_register.php?<?php echo $qstr ?>" class="btn btn_01">신규 식음료 등록</a>
</div>
-->
...
function beverageList_submit(f){
    ...
    if(document.pressed == "선택품절") {
        if(!confirm("선택한 식음료를 정말 품절시키겠습니까?\n\n품절되면 지점에서 주문할수 없게 됩니다")) {
            return false;
        }
    }
