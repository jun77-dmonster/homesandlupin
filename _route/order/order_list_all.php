...
$b1 = sql_query("select branch_cd, branch_nm from {$DM['BRANCH_TABLE']} where branch_withdrawal_fl='F' order by uid asc");

for ($i=0; $c1=sql_fetch_array($b1); $i++)
{
    $branch .= "<option value=\"{$c1['branch_cd']}\">$nbsp{$c1['branch_nm']}</option>\n";
}
			...
			<tr>
				...
				<td>
					<div class="inputbox-wrap">
						<select name="sfl_branch" style='width:300px;'>
							<?php echo conv_selected_option2($branch, $sfl_branch); ?>
						</select>
					</div>
				</td>
