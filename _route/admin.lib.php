...
function get_member_id_select($name, $level, $selected="", $event="")
{
    ...
    $sql = " select manager_id from {$DM['MANAGER_TABLE']} where manager_level >= '{$level}' ";
    $result = sql_query($sql);
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'><option value="">선택안함</option>';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['manager_id'].'"';
        if ($row['manager_id'] == $selected) $str .= ' selected';
        $str .= '>'.$row['manager_id'].'</option>';
    }
    $str .= '</select>';
    return $str;
}
...
function icon($act, $link='', $target='_parent')
{
    ...
    $icon = '<img src="'.G5_ROUTE_PATH.'/img/icon_'.$img[$act].'.gif" title="'.$act.'">';
    if ($link)
...
function admin_referer_check($return=false)
{
    ...
    if( $p['path'] && ! preg_match( '/\/'.preg_quote(G5_ROUTE_DIR).'\//i', $p['path'] ) ){
        $msg = '올바른 방법으로 이용해 주십시오';
    }
...
{
	if($is_branch=="super"){
		alert('지점 관리자는 최고 관리자에 접근이 안됩니다', '/_branch/');
	}else{
		alert('로그인 하십시오.', G5_BBS_URL.'/route.php?url=' . urlencode(correct_goto_url(G5_ROUTE_URL)));
	}
	
	//
}
else if ($is_route != 'super')
{
    $auth = array();
    $sql = " select au_menu, au_auth from {$DM['MANAGER_AUTH_TABLE']} where manager_id = '{$manager['manager_id']}' ";
    $result = sql_query($sql);
    for($i=0; $row=sql_fetch_array($result); $i++)
    {
...
$admin_key = md5($manager['manager_reg_dt'] . get_real_client_ip() . $_SERVER['HTTP_USER_AGENT']);
if (get_session('ss_manager_key') !== $admin_key) {
...
while ($entry = $tmp->read()) {
    ...
    $amenu[$m[1]] = $entry;
    $menu_files[] = G5_ROUTE_PATH.'/'.$entry;
}
@asort($menu_files);
