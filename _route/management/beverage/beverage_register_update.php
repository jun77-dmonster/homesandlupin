...
$beverage_driect_order	=	!empty($_POST['beverage_driect_order']) ? "T" : "F";


if (!$beverage_cd) { alert('식음료 코드는 반드시 입력하세요.'); }
if (!preg_match("/^([A-Za-z0-9_]{1,20})$/", $beverage_cd)) { alert('식음료코드는 공백없이 영문자, 숫자, _ 만 사용 가능합니다. (20자 이내)'); }

if (!$beverage_kor_nm || !$beverage_eng_nm) { alert('식음료 이름을 입력하세요.'); }

if($beverage_price==0){ alert('음료 가격은 0보다 커야 합니다'); }
...
if($w=="u"){
	...
	$beverage_file	=	$file['beverage_file'];
...
if ($w == "u") {
    goto_url("./beverage_register.php?w=u&amp;beverage_cd=$beverage_cd&amp;$qstr");
} else if ($w == "d")  {
...
<script>
    if (confirm("계속 입력하시겠습니까?"))
        ...
        location.href = "<?php echo "./beverage_list.php?".str_replace('&amp;', '&', $qstr); ?>";
</script>