			...
			<div class="content_item_bx">
				...
				<div class="tbl_head01 tbl_wrap">
					...
					for ($i=0; $row=sql_fetch_array($result); $i++) {
						$s_mod = "<a href='./beverage_register.php?w=u&amp;beverage_cd={$row['beverage_cd']}' class='board_copy btn btn_02'>수정</a>";
						$s_test = "<a href='./beverage_order.php?beverage_cd={$row['beverage_cd']}' class='board_copy btn btn_02'>주문테스트</a>";
					...
					<tr>
						...
						<td class="td_img2"><img src="<?php echo $beverage_img?>" style='width:80px;'></td>
						...
						<td class="td_category">
							<select name="beverage_driect_order[<?php echo $i?>]">
								<option value="T" <?php echo ($row['beverage_driect_order']=="T")?"selected":""?>>카운터주문</option>
								<option value="F" <?php echo ($row['beverage_driect_order']=="F")?"selected":""?>>태블릿주문</option>
							</select>
						</td>
						...
						<td class="td_mng_s"><?php echo ($row['beverage_display_fl']=="T")?"출력":"미출력"?></td>
...
function beverageList_submit(f){
    ...
    if(document.pressed == "선택삭제") {
        if(!confirm("선택한 식음료를 정말 삭제시키겠습니까?\n\n삭제되면 모든 지점에서 주문할수 없게 됩니다")) {
            return false;
        }
    }
