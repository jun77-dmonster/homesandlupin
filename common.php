...
// 운영자 로그인
if (isset($_SESSION['ss_manager_id']) && $_SESSION['ss_manager_id']) { // 로그인중이라면
	
	$manager = get_manager($_SESSION['ss_manager_id']);

	 // 오늘 처음 로그인 이라면
	if (substr($manager['last_login_dt'], 0, 10) != G5_TIME_YMD) {
		
		// 오늘의 로그인이 될 수도 있으며 마지막 로그인일 수도 있음
		// 해당 회원의 접근일시와 IP 를 저장
		$sql = " update DM_T_MANAGER set last_login_dt = '".G5_TIME_YMDHIS."', last_login_ip = '{$_SERVER['REMOTE_ADDR']}' where manager_id = '{$manager['manager_id']}' ";
		sql_query($sql);
	}
...
if (isset($_SESSION['ss_branch_id']) && $_SESSION['ss_branch_id']) { // 로그인중이라면
	
	$branch = get_branch_id($_SESSION['ss_branch_id']);

	 // 오늘 처음 로그인 이라면
	if (substr($branch['branch_login_dt'], 0, 10) != G5_TIME_YMD) {
		
		// 오늘의 로그인이 될 수도 있으며 마지막 로그인일 수도 있음
		// 해당 회원의 접근일시와 IP 를 저장
		$sql = " update DM_T_BRANCH set branch_login_dt = '".G5_TIME_YMDHIS."', branch_login_ip = '{$_SERVER['REMOTE_ADDR']}' where branch_manager_id = '{$branch['branch_manager_id']}' ";
		sql_query($sql);
	}
...
$is_route = '';
if (isset($manager['manager_id']) && $manager['manager_id']) {
    $is_member = true;
    $is_route = is_route($manager['manager_id']);
} 
